...
@export_range(0.01, 2.0, 0.01) var mouse_look_sensitivity = 0.2
@export_range(0.1, 3.0, 0.1) var joy_look_sensitivity = 0.5
@export_range(0.0, 0.99, 0.01) var controller_look_smoothing := 0.8

@export_range(32.0, 0.1) var speed := 15.0
@export_range(0.0, 100.0) var jump_strength = 26.0
@export_range(-5.0, 5.0) var gravity := -0.98

## How fast you accelerate, with 1 being immediate.
@export_range(1.0, 0.001) var acceleration := 0.2
## How many times faster your sprinting speed is. Will always be lerped by acceleration.
@export_range(3.0, 0.01) var sprint_multiplier = 1.8
## How smooth it is to go up and down stairs, with 1 being immediate.
@export_range(0.01, 1.0) var stair_speed = 0.3

## How many frames you have to make a late jump.
@export_range(0, 60) var coyote_time := 30
...
func rotate_head(look_x: float, look_y: float, has_smoothing: bool) -> void:
	if has_smoothing:
		...
		rotate_y(deg_to_rad(x_look_smoothing))
		y_look_smoothing = lerp(y_look_smoothing, -look_y, 1.0 - controller_look_smoothing)
		look_target.rotate_x(deg_to_rad(y_look_smoothing))
	else:
		rotate_y(deg_to_rad(-look_x))
		look_target.rotate_x(deg_to_rad(-look_y))
